...
export const useFileUpload = (options: FileUploadOptions = {}): [FileUploadState, FileUploadActions] => {
    ...
    (file: File | FileMetadata): string | null => {
      if (file instanceof File) {
        if (file.size > maxSize) {
          return `"${file.name}" dosyası maksimum ${formatBytes(maxSize)} boyutunu aşıyor.`;
        }
      } else {
        if (file.size > maxSize) {
          return `"${file.name}" dosyası maksimum ${formatBytes(maxSize)} boyutunu aşıyor.`;
      ...
      if (accept !== '*') {
        ...
        if (!isAccepted) {
          return `"${file instanceof File ? file.name : file.name}" kabul edilen bir dosya türü değil.`;
        }
      }
  ...
  const clearFiles = useCallback(() => {
    setState((prev) => {
      ...
      // onFilesChange'i setTimeout ile çağır
      setTimeout(() => {
    ...
    (newFiles: FileList | File[]) => {
      ...
      for (const file of newFilesArray) {
        ...
        if (file.size > maxSize) {
          errors.push(
            multiple
              ? `Bazı dosyalar maksimum ${formatBytes(maxSize)} boyutunu aşıyor.`
              : `Dosya maksimum ${formatBytes(maxSize)} boyutunu aşıyor.`,
          );
          continue;
        }
