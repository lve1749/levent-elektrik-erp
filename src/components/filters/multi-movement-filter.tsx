...
export type MovementStatus = 'active' | 'slow' | 'stagnant' | 'dead'

interface MultiMovementFilterProps {
  selectedStatuses: MovementStatus[]
  onChange: (statuses: MovementStatus[]) => void
}

const movementOptions: { value: MovementStatus; label: string; color: string }[] = [
  { value: 'active', label: 'Aktif', color: 'bg-green-500 dark:bg-green-400' },
  { value: 'slow', label: 'Yavaş', color: 'bg-yellow-500 dark:bg-yellow-400' },
  { value: 'stagnant', label: 'Durgun', color: 'bg-orange-500 dark:bg-orange-400' },
  { value: 'dead', label: 'Ölü Stok', color: 'bg-red-500 dark:bg-red-400' },
]

export default function MultiMovementFilter({ selectedStatuses, onChange }: MultiMovementFilterProps) {
  const [open, setOpen] = React.useState(false)

  const handleToggle = (status: MovementStatus) => {
  ...
  const handleSelectAll = () => {
    onChange(movementOptions.map(opt => opt.value))
  }

  const getDisplayContent = () => {
    if (selectedStatuses.length === 0) {
      return <span>Hareket Durumu</span>
    }
    if (selectedStatuses.length === movementOptions.length) {
      return <span>Tüm Durumlar</span>
    }
    if (selectedStatuses.length === 1) {
      const status = movementOptions.find(opt => opt.value === selectedStatuses[0])
      if (status) {
        return (
    ...
    return <span>{selectedStatuses.length} durum seçili</span>
  }
